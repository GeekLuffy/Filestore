    ...
    if len(text) > 7:
        ...
        if len(argument) == 3:
            ...
            try:
                ...
                return
        temp_msg = await message.reply("<code>A moment...</code>")
        try:
...
async def get_users(client: Bot, message: Message):
    ...
    await msg.edit(f"{len(users)} <b>using this bot</b>")
...
Deleted Account: <code>{deleted}</code></b>"""
        ...
        msg = await message.reply(
            "<code>This command must be sent while replying to the telegram message you want to broadcast.</code>"
        )
